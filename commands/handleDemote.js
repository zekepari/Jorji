...
import { guildIdTable, demotionTable } from '../config.js';

async function handleDemoteCommand(interaction, guildId) {
    ...
    if (currentRankIndex === 0) {
        return reply(interaction, `Cannot demote user ${actualUsername} further.`);
    }

    const previousRank = groupRoles[currentRankIndex - 1];

    const maxDemotableRanks = demotionTable[group.name][userRankInfo.rankName];
    if (!isServerOwner && (!maxDemotableRanks || !maxDemotableRanks.includes(previousRank.name))) {
        return reply(interaction, `You do not have permission to demote ${actualUsername} to ${previousRank.name}.`);
    }

    await assignRole(interaction, group.groupId, targetUserId, previousRank.name, targetUserRankLevel, actualUsername);
}
...
async function assignRole(interaction, groupId, targetUserId, roleName, currentRankLevel, username) {
    try {
        ...
        await reply(interaction, `User ${username} has been demoted to ${roleName}.`);
    } catch (error) {
        console.error(error);
...
export default handleDemoteCommand;
